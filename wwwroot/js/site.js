    ...
    $('#order_configure_form').on('submit', (evt) => {
      ...
      sendAjaxForm('', 'CheckOffers', (response) => {
        ...
        attributes.forEach(attr => {
          ...
          try {
            attributes = Array.from(document.getElementById('deliveryTariff').childNodes[document.getElementById('deliveryTariff').selectedIndex].attributes);
            attributes.forEach((a, i) => {
              if (a.name.includes('tariff-')) {
                data.append(`delivery.${a.name.replace('tariff-', '')}`, a.value);
              }
            });
          } catch (e) {
            console.log(e);
